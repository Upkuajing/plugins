...
Search people data from LinkedIn using the UpKuaJing Open Platform API.

## Overview

This skill provides access to UpKuaJing's LinkedIn people data through one list search script. API key generation and top-up are provided through the `auth.py` script.
...
**Important**: Always use direct script invocation like `python scripts/linkedin_person_search.py`. **Do NOT use** shell compound commands like `cd scripts && python linkedin_person_search.py`

### LinkedIn Person List Search (`linkedin_person_search.py`)
- **Return granularity**: Each person as one record
- **Use cases**: Focus on "which people exist" in LinkedIn data
- **Examples**:
  - "Find CTOs at XXXX"
  - "Find Sales Directors in China"
- **Parameters**: See [LinkedIn Person List](references/linkedin-person-list-api.md)
...
| User Intent | Use API |
|-------------|---------|
| "Find CTOs of XX company" | LinkedIn person list (companyNames + titleNames) |
| "Find people with email in US" | LinkedIn person list existEmail=1 + countryCodes=["US"] |
| "Find people in a specific industry" | LinkedIn person list humanIndustries |
...
```bash
python scripts/linkedin_person_search.py \
  --params '{"companyNames": ["XXXX"], "titleNames": ["CTO"]}' \
  --query_count 20
```
...
```bash
python scripts/linkedin_person_search.py --params '{"humanIndustries": ["technology"], "countryCodes": ["US"], "existEmail": 1}' --query_count 1000
```
...
```bash
python scripts/linkedin_person_search.py --task_id 'task-id-here' --query_count 2000
```
...
1. **Check API documentation**:
   ...
   - Check [references/linkedin-person-list-api.md](references/linkedin-person-list-api.md)
   - Do not guess parameter names, get accurate parameter names and formats from documentation

2. **Optimize query parameters**:
   - Use `humanIndustries` / `companyNames` to filter precisely; industry names must be in **English**
