...
"""
领英找人列表搜索
数据来源已由接口固定为 linkedin，通过职位、公司、行业等信息搜索人物，
...
# 接口端点（source 已固定为 linkedin）
ENDPOINT = '/agent/search/linkedin/person/list'


def search_person_list(params: dict, cursor: str = None, pit_id: str = None) -> dict:
    """
    使用游标搜索领英人物列表。
...
def main():
    parser = argparse.ArgumentParser(
        description='从跨境魔方开放平台搜索领英人物信息（source 固定为 linkedin）'
    )
    parser.add_argument(
        '--params',
        help='搜索参数JSON字符串'
    ...
    while total_retrieved < args.query_count:
        try:
            # 搜索当前批次
            response = search_person_list(params, current_cursor, current_pit_id)
            ...
            # 保存数据到文件
            if person_list:
                append_result_data(task_id, person_list)
                total_retrieved += len(person_list)
            ...
            is_completed = (total_retrieved >= args.query_count) or len(person_list) == 0 or (not current_cursor)
