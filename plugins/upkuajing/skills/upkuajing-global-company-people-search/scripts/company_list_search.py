...
def search_company_list(params: dict, cursor: str = None) -> dict:
    """
    使用游标搜索公司列表。
    ...
    # 发起API请求
    response = make_request('/agent/search/company/list', request_params)
    return response


def main():
    parser = argparse.ArgumentParser(
        description='从跨境魔方开放平台搜索公司信息'
    )
    parser.add_argument(
        '--params',
        help='搜索参数JSON字符串'
    ...
    while total_retrieved < args.query_count:
        try:
            # 搜索当前批次
            response = search_company_list(params, current_cursor)
